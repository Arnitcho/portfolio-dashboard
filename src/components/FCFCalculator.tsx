...
export default function FCFCalculator({ defaultPrice, defaultFCF, defaultG, currency = 'USD' }: FCFCalculatorProps) {
  ...
  return (
    <div>
      {/* Result header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', padding: '16px 20px', background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.15)', marginBottom: '20px' }}>
        <div>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: 'rgba(240,236,224,0.4)', letterSpacing: '0.1em', marginBottom: '4px' }}>
            PRIX CIBLE FCF
          </div>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '20px', fontWeight: 300, color, marginBottom: '4px' }}>
            {upside >= 0 ? '+' : ''}{upside.toFixed(1)}%
          </div>
          <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', letterSpacing: '0.1em', color, background: `${color}15`, padding: '3px 8px', border: `1px solid ${color}30` }}>
            {label}
          </div>
        </div>
      </div>
        ...
        {sliders.map(s => (
          <div key={s.label}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '12px', color: 'rgba(240,236,224,0.6)' }}>
                {s.label}
              </span>
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '12px', color: '#c9a84c' }}>
                {s.format(s.value)}
              </span>
            </div>
            <input
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: 'rgba(240,236,224,0.25)' }}>
                {s.format(s.min)}
              </span>
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '9px', color: 'rgba(240,236,224,0.25)' }}>
                {s.format(s.max)}
      ...
      {/* Formula note */}
      <div style={{ marginTop: '16px', padding: '10px 14px', background: 'rgba(201,168,76,0.03)', border: '1px solid rgba(201,168,76,0.1)' }}>
